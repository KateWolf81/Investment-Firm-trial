...
st.caption(
    ...
    "its own call to Claude, so this genuinely takes 5-10 minutes. Leave this tab open and "
    "your computer awake until it finishes; closing the tab or rebooting the app mid-run will "
    "interrupt it. Once it's done, the report is saved — safe to close and check back later."
)
...
# If this session doesn't remember a report (e.g. reopened later, or after a reboot)
# but one was already generated, pick up the most recent file from disk instead of
# losing it — this is what makes "start it, check back later" actually work.
